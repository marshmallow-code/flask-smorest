...
class TestApi():
    ...
    def test_init_multiple_apps(self):
        """Test multiple apps can be initialized"""
        app_1 = Flask('App 1')
        app_2 = Flask('App 2')
        api = Api()

        blp_1 = Blueprint('test', 'test')
        blp_2 = Blueprint('test', 'test')
        ...
        api.init_app(app_1)
        api.register_blueprint(app_1, blp_1)
        api.init_app(app_2)
        api.register_blueprint(app_2, blp_2)

        assert set(api._specs[app_1].to_dict()['paths']) == {'/test_1'}
        assert set(api._specs[app_2].to_dict()['paths']) == {'/test_2'}
